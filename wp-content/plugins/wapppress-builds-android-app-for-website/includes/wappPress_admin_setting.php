...
class wappPress_admin_setting extends wappPress {
	...
	$options = get_option('wapppress_settings');
		...
		if (preg_match("/^(\w{8})-((\w{4})-){3}(\w{12})$/", $options['wapppress_pro_license']))
	{
								...
								<form method="post" action="options.php">
									...
									<div id="tab1" class="tab active">
										...
										<?php
											...
											<h3 style="background:#ef9b02">
											<?php if (preg_match("/^(\w{8})-((\w{4})-){3}(\w{12})$/", $options['wapppress_pro_license']))
												{
													echo "Enter Purchase Code";
												}else{
													echo "To Upgrade to Pro Version, Enter Purchase Code";
												}?>
											</h3>
											...
											<div class="panel">
												...
												<p> Enter (Codecanyon)Purchase Code([Optional] only for Pro Users).</p>

												<p>

													<?php $frontpage_id1 =  get_option('page_on_front'); 

													if($frontpage_id1 !=@$options['wapppress_pro_license']){

													?>

													<input class="app_input"  type="text" id="wapppress_pro_license" name="wapppress_settings[wapppress_pro_license]" value="<?php echo @$options['wapppress_pro_license']; ?>" />

													<?php }else{ ?>

													<input class="app_input"  type="text" id="wapppress_pro_license" name="wapppress_settings[wapppress_pro_license]" value="" />
					...
					<div class="setting-sec">
						...
						<div class="setting-form">
							...
							<br />
								...
								<input type="hidden" name='license' id='license' placeholder='CodeCanyon "Item Purchase Code"' value="<?php echo @$options['wapppress_pro_license']; ?>"  />
...
<?php
	...
	function settings_validate($arr_input) {
		...
		@$purchase_code = trim( $arr_input['wapppress_pro_license'] );
		if (preg_match("/^(\w{8})-((\w{4})-){3}(\w{12})$/", $purchase_code))
			{
				
				@$options['wapppress_pro_license'] =$purchase_code;
			}else{
				@$options['wapppress_pro_license'] ='Invalid Code';
			}
...
$push_icon_name1='ic_stat_gcm.png';
	...
	if ( $_FILES['app_logo']['error'] === UPLOAD_ERR_OK ) {
		...
		$app_logo_size = $_FILES['app_logo']['size'];
		$app_logo_type = $_FILES['app_logo']['type'];
		// File information 
		$app_logo_ext  = substr( $app_logo_name, strrpos( $app_logo_name, '.' ) + 1); 
		// Is it an image? 
		if(   (strtolower($app_logo_ext ) == 'png' ) && ( $app_logo_size < 2097152  ) && (  $app_logo_type == 'image/png' ) && 
			getimagesize( $app_logo_temp ) ) 
			{ 
			//continue
			
			}else{
				exit;//not a logo image
			}
		...
		if ( $_FILES['app_splash_image']['error'] === UPLOAD_ERR_OK )
			{
				...
				$app_splash_size = $_FILES['app_splash_image']['size'];
				$app_splash_type = $_FILES['app_splash_image']['type'];
				// File information 
				$app_splash_ext  = substr( $app_splash_image, strrpos( $app_splash_image, '.' ) + 1); 
				// Is it an image? 
				if( (  strtolower( $app_splash_ext ) == 'png' ) && ( $app_splash_size < 2097152  ) && (  $app_splash_type == 'image/png' ) && 
					getimagesize( $app_splash_temp ) ) 
					{ 
					//continue
					
					}else{
						exit;//not a splash image
					}
